...
img_name_suffix = "t2.nii.gz"
gt_name_suffix = "GlistrBoost.nii.gz"
prefix = modality + "_" + anatomy + "_"

nii_path = "E:/SAM_code/BRATS2017/Pre-operative_TCGA_GBM_NIfTI_and_Segmentations/"  # path to the nii images
gt_path = "E:/SAM_code/BRATS2017/Pre-operative_TCGA_GBM_NIfTI_and_Segmentations/"  # path to the ground truth
...
for name in tqdm(names):
    for n in os.listdir(join(gt_path, name)):
        if img_name_suffix in join(nii_path, name, n):
            names_list.append(join(nii_path, name, n))
...
for name in tqdm(names_list):  # use the remaining 10 cases for validation
    # print(name)
    image_name = name#name.split(gt_name_suffix)[0] + img_name_suffix
    gt_name = name.replace(img_name_suffix,gt_name_suffix)
    gt_sitk = sitk.ReadImage(gt_name)
    ...
    if len(z_index) > 0:
        ...
        # print(gt_name)
        # print(join(npy_path, prefix + gt_name.split(gt_name_suffix)[0].split("\\")[-1]+'.npz'))
        np.savez_compressed(join(npy_path, prefix + gt_name.split(gt_name_suffix)[0].split("\\")[-1]+'.npz'), imgs=img_roi, gts=gt_roi, spacing=img_sitk.GetSpacing())
        ...
        sitk.WriteImage(
            img_roi_sitk,
            join(npy_path, prefix + gt_name.split(gt_name_suffix)[0].split("\\")[-1] + 'img.nii.gz')
            # join(npy_path, prefix + gt_name.split(gt_name_suffix)[0] + "_img.nii.gz"),
        )
        sitk.WriteImage(
            gt_roi_sitk,
            join(npy_path, prefix + gt_name.split(gt_name_suffix)[0].split("\\")[-1] + 'gt.nii.gz')
        ...
        for i in range(img_roi.shape[0]):
            ...
            np.save(
                join(
                    ...
                    "imgs",
                    prefix
                    + gt_name.split(gt_name_suffix)[0].split("\\")[-1]
                    + "-"
                    + str(i).zfill(3)
                    + ".npy",
                ),
